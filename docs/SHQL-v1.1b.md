# ShorthandQL v1.1 beta (SHQL-v1.1b) — Loadable Language Definition

**Name:** ShorthandQL version 1.1 beta  
**File:** `SHQL-v1.1b.md`  
**Status:** Project-local loadable definition (Project Newborn)  
**Activation:** **Opt-in.** Default OFF in sessions. User turns ON/OFF via `AGENTS.md` toggles (`::@load::SHQL-v1.1b.md/`, “turn on/off shorthand”, `::@unload/`, etc.).  
...
| `::@load::SHQL-v1.1b.md/` | Load this definition (aliases: `spec`, `SHQL-v1.1b`, `SHQL-v1.1b.md`) | `Loaded ShorthandQL v1.1 beta.` |
| `::@unload/` | Unload / turn OFF this definition (optional filename arg) | `Unloaded ShorthandQL v1.1 beta.` |
...
8. If first segment starts with `@`:  
   ...
   - If `cmd` ∈ {list, add, remove, save, preset, load, unload, clear, help, lock, unlock, temperature, context, effort, restore, return} **or** `cmd` is `capture` / starts with `capture-` **or** `cmd` is `scrape` / starts with `scrape-` **or** `cmd` is `compact` / starts with `compact-` → **Command** (validate required args).  
...
5. **MalformedCommand** — bad/unknown `@…` or strength on a command name  
   `\`@save^2\` is not a valid command. Valid commands are: list, add, remove, save, preset, load, unload, clear, help, lock, unlock, capture, temperature, context, effort, scrape, compact, restore, return. Strength modifiers apply only to domain tags. (DADO is a plain-word acronym, not an @-command.)`
...
1. Always recognize **turn ON / turn OFF** phrases and `::@load::SHQL-v1.1b.md/` / `::@unload/` (and aliases).  
...
4. While OFF, `::@help/` may list: load/unload toggles + pointer to `SHQL-v1.1b.md`. While ON, include `effort (1–5 | default | clear)` and `scrape [-dN] [selectors] / URL`.  
...
## 9. Design Notes (v1.1 beta — for reviewers)
...
**Not in this v1.1 beta (future)**  
- Rulelists (`::@rule-N::`), macros language surface, fixed tag vocabulary.

---

**End of ShorthandQL v1.1 beta (`SHQL-v1.1b.md`)**
